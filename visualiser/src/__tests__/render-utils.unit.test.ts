...
describe("teamColor", () => {
  ...
  it("may return different colors for different teams", () => {
    ...
    const colors = new Set(["alpha", "beta", "gamma", "delta", "epsilon"].map(teamColor));
...
describe("computeBounds", () => {
  ...
  it("includes clusters in bounds", () => {
    const layout: LayoutResult = {
      ...
      edges: [],
      clusters: [{ filePath: "f.ts", x: 0, y: 0, width: 200, height: 200 }],
      clusterOrder: [],
    };
    const bounds = computeBounds(layout);
...
describe("filterByFocus", () => {
  const data: GraphData = {
    nodes: [makeGraphNode({ id: "a" }), makeGraphNode({ id: "b" }), makeGraphNode({ id: "c" })],
