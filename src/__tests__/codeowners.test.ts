...
describe('getFileOwners', () => {
  ...
  it('matches recursive glob patterns', () => {
    const owners = getFileOwners(
      'workspaces/domain/service/BartService.ts',
      rules
    );
  ...
  it('handles patterns with leading slash', () => {
    const rules: CodeownersRule[] = [
      { pattern: '/src/config.ts', teams: ['@org/infra'] },
    ];
  ...
  it('handles multiple team owners', () => {
    const rules: CodeownersRule[] = [
      { pattern: 'shared/', teams: ['@org/team-a', '@org/team-b'] },
    ];
...
describe('buildCodeownersMap', () => {
  it('builds map with only matched files', () => {
    const rules: CodeownersRule[] = [
      { pattern: 'src/', teams: ['@org/frontend'] },
    ];
    const map = buildCodeownersMap(
      ['src/app.ts', 'src/utils.ts', 'lib/other.ts'],
      rules
    );
    expect(map).toEqual({
      'src/app.ts': ['frontend'],
